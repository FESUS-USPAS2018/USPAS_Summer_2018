...
class ParticleTracker():
  ...
  def __init__(self,particles,indices):
    self.particles = particles
    self.indices = indices
    self.n = len(indices)

    #Initialize the container to keep track of the coordinates.
    self.container = []
    for i in indices:
      self.container.append([])

  def recordPosition(self):
    x = self.particles.getx()
    y = self.particles.gety()
    z = self.particles.getz()
    for j in range(self.n):
      i = self.indices[j]
      self.container[j].append((x[i],y[i],z[i]))
  ...
  def report(self,i,filepath):
    j = self.getIndex(i)
    with open(filepath,'w') as f:
      for k in range(len(self.container[j])):
        output = ",".join([str(x) for x in self.container[j][k]])
        f.write(output+"\n")
